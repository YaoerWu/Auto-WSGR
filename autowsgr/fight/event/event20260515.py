...
EVENT = Path(__file__).stem[-8:]
assert EVENT == '20260515'
...
class EventFightPlan(Event, BattlePlan):
    def __init__(
        self,
        timer: Timer,
        plan_path,
        ...
        self.info = EventFightInfo(timer, self.config.chapter, self.config.map)
        Event.__init__(self, timer, EVENT)

    def _load_fight_info(self):
        self.info = EventFightInfo(self.timer, self.config.chapter, self.config.map)
...
class EventFightInfo(Event, BattleInfo):
